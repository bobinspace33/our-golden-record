...
/** Shown at the start of every AI council member response (not human seats). */
const AI_COUNCIL_MEMBER_DISCLAIMER =
  "Before we get started, remember that my response is generated by AI and can contain mistakes. Check any sources I provide and consult your instructor for additional guidance.";

function councilMemberIsAi(gemOrId) {
  if (APP_KIND !== "custom") return true;
  const g =
    typeof gemOrId === "object" && gemOrId != null
      ? gemOrId
      : gems.find((x) => Number(x.id) === Number(gemOrId));
  return !g?.isHuman;
}

function createAiResponseDisclaimerElement() {
  const p = document.createElement("p");
  p.className = "ai-response-disclaimer";
  p.textContent = AI_COUNCIL_MEMBER_DISCLAIMER;
  return p;
}

function aiResponseDisclaimerHtml() {
  return `<p class="ai-response-disclaimer">${escapeHtml(AI_COUNCIL_MEMBER_DISCLAIMER)}</p>`;
}

function ensureAiResponseDisclaimerBefore(textEl, isAiMember) {
  if (!textEl || !isAiMember) return;
  const body = textEl.parentElement;
  if (!body || body.querySelector(":scope > .ai-response-disclaimer")) return;
  body.insertBefore(createAiResponseDisclaimerElement(), textEl);
}

async function displayCouncilResponseText(textEl, text, { animate = true, isAiMember = true, animation } = {}) {
  if (!textEl) return;
  ensureAiResponseDisclaimerBefore(textEl, isAiMember);
  textEl.classList.remove("response-overlay-text--waiting", "response-overlay-text--thinking");
  const anim = animation || getResponseAnimationParams();
  if (animate) await animateResponseText(textEl, text || "", anim);
  else renderResponseTextStatic(textEl, text || "");
}
...
async function finishCouncilOverlayCard(gemId, result, { jobTitleMap = {}, animate = true, showSaveButton = true }) {
  ...
  if (result.error) {
    ...
    textEl.classList.remove("response-overlay-text--waiting", "response-overlay-text--thinking");
    await displayCouncilResponseText(textEl, result.response || "", {
      animate,
      isAiMember: councilMemberIsAi(gFound),
...
function appendFollowUpToCard(card, followUpText) {
  ...
  if (textEl) {
    ensureAiResponseDisclaimerBefore(textEl, true);
    animateResponseText(textEl, followUpText, getResponseAnimationParams());
  }
}
...
function openResponsesOverlay(results, options = {}) {
  ...
  results.forEach(({ gemId, name, response, error, jobTitle }) => {
    ...
    if (error) {
      ...
      void displayCouncilResponseText(textEl, response, {
        animate,
        isAiMember: councilMemberIsAi(gFound),
        animation: animParams,
      });
    ...
    if (followUp && followUp.length > 0) {
      ...
      followUp.forEach((r) => {
        ...
        const followDisc = councilMemberIsAi(rGem) ? aiResponseDisclaimerHtml() : "";
        fc.innerHTML = `
...
function renderResults(results, options = {}) {
  ...
  results.forEach(({ gemId, name, response, error, jobTitle }) => {
    ...
    if (error) {
      ...
      const disclaimerHtml = councilMemberIsAi(gFound) ? aiResponseDisclaimerHtml() : "";
      card.innerHTML = `
        <h3>${escapeHtml(headerName)}</h3>
        ${title ? `<p class="result-job-title">${escapeHtml(title)}</p>` : ""}
        ${disclaimerHtml}
